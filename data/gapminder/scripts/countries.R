...
df <- 
  ...
  select(
    ...
    income_wb_2017 = "World bank income group 2017"
  ) %>% 
...
stopifnot(
  ...
  n_distinct(df$iso_a3) == nrow(df),
  n_distinct(df$name) == nrow(df),
  n_distinct(df$region_gm4) == 4,
  n_distinct(df$region_gm6) == 6,
  n_distinct(df$region_gm8) == 8,
  all(df$oecd_g77 %in% c("oecd", "g77", NA)),
  all(df$un_status %in% c("member", "observer", "non-member"))
)
