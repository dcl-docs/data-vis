...
buildings_10 %>% 
  ...
  ggplot(mapping = aes(x = building, y = height)) +
  geom_point() +
  coord_flip()
```
...
buildings_10 %>% 
  mutate(building = fct_reorder(building, height)) %>% 
  ggplot(mapping = aes(x = building, y = 1, color = height)) +
  geom_point(size = 10) +
  coord_flip() +
  theme(
...
buildings_10 %>% 
  mutate(building = fct_reorder(building, height)) %>% 
  ggplot(mapping = aes(x = building, y = 1, size = height)) +
  geom_point() +
  coord_flip() +
  theme(
    axis.text.x = element_blank(),
...
buildings_10 %>% 
  mutate(building = fct_reorder(building, height)) %>% 
  ggplot(mapping = aes(x = building, y = height, size = floors)) +
  geom_point() +
  coord_flip()
```
...
If this were an EDA plot, you might further investigate this discovery about Tianjin CTF Finance Centre by directly plotting the `height` to `floors` ratio. 

```{r}
buildings_10 %>% 
  mutate(building = fct_reorder(building, height / floors)) %>% 
  ggplot(mapping = aes(x = building, y = height / floors)) +
  geom_point() +
  coord_flip()
```
...
```{r}
buildings %>% 
  ggplot(mapping = aes(x = country, y = height)) +
  geom_point() +
  coord_flip() +
  stamp("Bad")
```
