...
Adding text to your visualizations can make them more informative. The following chapter from _R4DS_ will introduce you to strategies for effective annotations.

* [Annotations](https://r4ds.had.co.nz/graphics-for-communication.html#annotations)

### Log scales

There are many different scale functions, and they don't all follow the same format. The following table summarizes the common scale functions.
...
Using a log scale for `population` spreads out the smaller countries, making their differences more obvious.
...
Use `scale_y_log10()` if you want to use a log scale for the y-axis.

Log scales are also useful for data that follow a power law. Power law relationships are fairly common. One example is word frequencies. Say you count the number of times each word appears in some text, then rank each word according to this frequency. For many collections of text, the frequency of each word is inversely proportional to its rank, and this relationship follows a power law.
...
Diverging color scales are useful if you want to encode both the sign and magnitude of each value. For example, if you have temperature data, you might want to encode if a temperature is above or below freezing, as well as how far below or above freezing. 
...
The default size scale function is `scale_size().` `scale_size()` scales circle areas, not radii because humans judge the size of circles based on area.
...
mpg %>% 
  ggplot(aes(hwy, cty, size = displ)) +
  geom_point() +
  scale_size(breaks = c(2, 4, 6), range = c(.5, 4))
```
...
There are several problems with this plot. First, most of the data is concentrated on the left edge of the plot because of a few large `gdp_per_capita` values. A log scale will help this problem.
...
Second, there is a lot data, and many of the points overlap. This makes it difficult to determine how many points there are in a given area. In the next section, we'll talk about strategies for dealing with overplotting.

### Overplotting 

One way to address overplotting is to bin the data and then encode the number of points in each bin with color. `geom_bin2d()` and `geom_hex()` both carry out this strategy.
...
We recommend using `geom_hex()` instead of `geom_bin2d()` in most situations. On average, hexagons more accurately reflect the underlying data than rectangles. 
...
Like `geom_histogram()`, `geom_bin2d()` and `geom_hex()` both create a default number of bins. You can adjust the number of bins or binwidth with the `bins` and `binwidth` arguments. In our example, however, the default binwidth does a good job.
...
Another option is change the shape of the dots so that they have visible borders. `geom_point()` points actually don't have to be solid-colored circles. There are 25 different possible shapes. The following image from the_Scales_ section of the [ggplot2 cheat sheet](https://github.com/rstudio/cheatsheets/blob/master/data-visualization-2.1.pdf) displays these shapes along with their identifying numbers.
...
The transparent interiors and black borders make it easier to peceive individual points and spot overlaps. This effect is stronger if you fill the circles and add a white border.
...
A simpler approach is to make the points more transparent. In `geom_point()`, you can adjust the transparency by adjusting `alpha`.
...
Adjusting `alpha` makes it both easier to see individual points and determine areas of high density.
...
What is the relationship between `life_expectancy` and `gdp_per_capita`? It entire blue blob appears somewhat linear, but if you look at just the areas with the highest density, you can see it curve upwards.
...
Smooth lines are a helpful tool, but they aren't always necessary. Smooth lines are a good idea when you care about understanding or communicating a trend, but that trend is not immediately obvious without a smooth line. In current example, because we used `geom_hex()` to visualize the underlying data, it is difficult to see the trend without a smooth line.
...
You can remove the confidence intervals by setting `se = FALSE` inside `geom_smooth()`, but you should only do so if the confidence intervals are very small and constant throughout the entire smooth line.

One part of decoding this plot involves connecting each line to its corresponding legend label. There are only four different colors, but you still have to go back-and-forth between the legend and the plot quite a bit. This process would be easier if the order of the legend matched the order of the lines (@wilke-2019). We'll do this by adjusting `breaks` in `scale_color_discrete()`.
...
It's now much easier to connect a line with a region. These adjustments aren't necessary for EDA plots, but they're helpful for presentation plots.
...
From this plot, you can tell that life exectancies didn't change much from 2010 to 2015 for most countries. A reference line will make it easier to tell if they stayed exactly the same, increased, or decreased.
...
gapminder_paired %>% 
  ...
  annotate(
    ...
    x = 30, 
    y = 85, 
    label = "Life expectancy increased", 
    hjust = 0
  ) +
  annotate(
    "text", 
    x = 65, 
    y = 50, 
    label = "Life expectancy decreased", 
    hjust = 0
  ) +
...
Notice that there a couple points far away from the red line. What countries do these points represent? Labelling these points would make our plot more interesting. 
...
gapminder_paired %>% 
  ggplot(aes(year_2010, year_2015)) +
  geom_point(alpha = 0.5) +
  geom_text_repel(aes(label = name), nudge_x = -5, data = outliers) +
  annotate(
    "text", 
    x = 30, 
    y = 85, 
...
Encoding both `year_2010` and `year_2015` with position make it difficult to estimate the exact change in life expectancy for a given country. If we want to highlight the exact change in life expectancy for each individual country, it would be better to directly encode the change by creating a new variable.
...
Our visualization is now a discrete-continuous problem. There are too many individual countries for us to be able to create a reasonably sized plot, so we'll just focus on Asia for now.
...
(We made the plot space bigger so that it could fit in all the countries in the region. You can do this by adjusting the `fig.height` parameter in an RMarkdown chunk. This chunk was set to `fig.height = 3`.)
...
The three different plots we made (the original scatterplot, the discrete-continuous one showing just `diff`, and this final one) all highlight different elements of the data. As always, the best visualization will depend on what you are trying to understand or communicate about your data. 
...
A scatterplot will use up our two "position along a common scale" options. We should encode the most important variables with position. Let's say those are `life_expectancy` and `per_capita_gdp`. We'll also encode our discrete variable, `region`, with color. 
...
gapminder_2015 %>% 
  ...
  geom_point(shape = 21, color = "black") +
  scale_size(range = c(1, 20), breaks = c(1e6, 1e8, 1e9)) +
  scale_x_log10() 
```
