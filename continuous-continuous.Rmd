...
Adding text to your visualizations can make them more informative. The following chapter from _R4DS_ will introduce you the mechanics of effective annotations.
...
read_csv("scale_table.csv") %>% 
  replace_na(list(discrete = " ")) %>% 
  knitr::kable()
```

Transforming linear scales into log scales is a common use of the scale functions. Log scales are helfpul in several different scenarios. 
...
We recommend using `geom_hex()` instead of `geom_bin2d()` in most situations. On average, hexagons more accurately reflect the underlying data. 
...
The transparent interiors and black borders make it easier to peceive individual points and spot areas of overlap. This effect is stronger if you fill the circles and add a white border.
...
Smooth lines are a helpful tool, but they aren't always necessary. Use mooth lines when you care about understanding or communicating a trend, but that trend is not immediately obvious without a smooth line.
...
It looks like, for most countries, life exectancy didn't change much from 2010 to 2015. A reference line will make it easier to tell if they stayed exactly the same, increased, or decreased.
...
(We made the plot space bigger by adjusting the `fig.height` parameter in the R Markdown chunk. This chunk was set to `fig.height = 3`.)
