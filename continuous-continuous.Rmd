...
# need to figure out why this isn't showing up
...
Notice how countries from Jordan to Micronesia appear to have the same population. We can use `scale_y_log10()` to specify a log scale. The differences between the smaller countries now become obvious.
...
Log scales are also useful if you suspect that your data follows an exponential, power law, or logarithmic relationship.
...
`scale_color_gradient2()` has three color set points that you can adjust: `low`, `mid`, and `high`.
...
Making the points more transparent is one solution to minor overplotting. In `geom_point()`, you adjust the transparency by setting `alpha` to a value less than 1.
...
Another option is to change the appearance of the points so that they have a border. There are many possible shapes `geom_point()` points can take. Each shape is associated with a number. The following image is from the _Scales_ section of the [ggplot2 cheat sheet](https://github.com/rstudio/cheatsheets/blob/master/data-visualization-2.1.pdf).
...
The transparent interiors and black borders allow you to perceive individual points and their overlaps better. This effect is stronger if you fill the circles and add a white border.
...
In this situation, the `alpha` solution is probably better. However, in situations in which you want to be able to distinguish each point, `shape = 21` can be helpful. You'll see an example of a such a situation later on in the chapter.

Another solution to overplotting is to _jitter_ the points using `geom_jitter()`. `geom_jitter()` adds some random variation to the position of each point so that they no longer overlap. `geom_jitter()` is generally only a good solution if your points lie directly on top of each other.

```{r}
v <-
  tibble(
    x = c(rep(1, 5), rep(2, 4), rep(1, 2), rep(3, 10)),
    y = c(rep(2, 5), rep(4, 4), rep(5, 2), rep(3, 10))
  )
...
Even in these cases, you should be cautious  about using `geom_jitter()`, since, by design, the position of the points no longer directly reflect the underlying data.
...
In this case, there's not much of a difference between the two methods. Generally, if you're using a smooth line, you don't need to worry too much about the specifics of the method. Smooth lines are a helpful visual and exploratory tool. You shouldn't rely on the them to infer the functional form of the data or predict on new data.

Use smooth lines when you care about understanding or communicating a trend, and that trend is not immediately obvious without a smooth line. In our current situation, the smooth line makes the relationship between `life_expectancy` and `gdp_per_capita` clearer. 
...
This plot does not work. There are too many data points to determine the relationship between `life_expectancy` and `gdp_per_capita` for each region. We could add a smooth line on top of the points for each region.
...
Sometimes, `geom_line()` will be appropriate for continuous-continuous relationships. The following plot shows the relationship between `life_expectancy` and `gdp_per_capita` for just the Scandinavian countries. 
...
You can see that each country has it's own trend. However, it is unnecessarily difficult to connect each group of dots. Adding lines visually connects the dots for you.
...
You can now see that Finland's life expectancy has changed more than that of the other countries. It's also interesting that Iceland and Sweden look very similar, and that Norway's per capita GDP pulled away from the rest, but its life expectancy remained similar.
...
To explicitly encode the change, we'll need to make a new variable.
...
However, now we can't tell if the larger circles cover up smaller ones. We could adjust `alpha` to address this overlap.
...
This is better. However, there's still the possibility that the large dots cover up smaller ones. One useful thing to know about `geom_point()` is that it plots points in order of their appearance in the data. If you want to change the order in which the points are plotted, you can change the order of the rows in your data.

We'll use `arrange()` to reorder our data so that the smallest countries are plotted on top of the larger ones.
...
Each dot is now more distinguishable. We can also try adding `fill` to encode `region` (recall that, unlike the default point shapes, `shape = 21` uses `fill` to control the interior of the points).
...
gapminder_2015 %>% 
  ...
  geom_point(shape = 21, color = "black") +
  scale_size(range = c(1, 20), breaks = c(1e6, 5e8, 1e9)) +
  scale_x_log10() 
```
